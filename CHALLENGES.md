...
3. Extend the category in `03_categories_and_lightweight_generics.m` with one more formatting helper.
...
1. In `01_model_mapping_and_validation.m`, support an optional `notes` field.
2. In `02_nsoperation_queue_pipeline.m`, add a fourth operation that depends on `render`.
3. In `03_file_persistence_and_serialization.m`, deserialize into model objects instead of raw dictionaries.
4. In `04_testing_mindset_with_assertions.m`, add two more assertions, including one edge case.

## Advanced

1. Add one more dynamic selector to `01_runtime_and_message_dispatch.m`.
2. Create an intentional retain-cycle example in `02_memory_management_and_blocks.m`, then fix it.
3. In the KVC/KVO lesson, track a second observable property.
4. Replace the scorer in `04_protocols_categories_and_service_design.m` with a second strategy implementation.
...
3. Add persistence using the JSON approach from `intermediate/03_file_persistence_and_serialization.m`.
